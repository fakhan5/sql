...
SELECT vendor_id, count(booth_number) AS num_of_booth_rented
...
CAST(sum(cp.quantity * cp.cost_to_customer_per_qty) as INTEGER) as price
FROM customer as c
...
*/
--check to see if temp database exists, if yes, clear from memory, else do nothing
DROP TABLE IF EXISTS temp_new_vendor;

--create new temp table
CREATE TEMP TABLE temp_new_vendor AS

--temp table created from existing vendor table
SELECT *
FROM vendor 

--insert new customer data into temp table, this query is seperate from the one above
INSERT INTO temp_new_vendor (vendor_id, vendor_name, vendor_type, vendor_owner_first_name, vendor_owner_last_name)
VALUES (10, 'Thomass Superfood Store', 'Fresh Focusued', 'Thomas', 'Rosenthal') 
...
, strftime ('%Y', market_date) as purchase_year
, strftime ('%m', market_date) as purchase_month
, sum(cost_to_customer_per_qty) as total_month_spend
FROM customer_purchases
WHERE purchase_year LIKE ('2022') 
AND purchase_month LIKE ('04')
